...
const electStub = {
  ipcMain: {
    handle: (channel: string, cb: Function) => { handlers[channel] = cb; },
...
class SessionRepositoryStub {
  state = {
    session: {
      id: 's1', initialIntent: 'improve', status: 'collecting', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), steps: [], selectedOptionIds: [], confidence: 0, pendingQuestionId: null
    }
  };
...
describe('Retry idempotence', () => {
  ...
  it('failed next-question attempt does not duplicate steps when retried and then succeeds', async () => {
    ...
    // First call throws
    jest.spyOn(OkrAgentService.prototype, 'getNextQuestion')
      .mockRejectedValueOnce(new Error('network'))
      .mockResolvedValueOnce({ question: { id: 'q2', text: 'Next', options: [{ id: 'a', label: 'A' }, { id: 'b', label: 'B' }] } });
    ...
    // First attempt fails
    await expect(h(null, { context: { turns: [] }, lastChoice: { questionId: 'q1', optionId: 'a' } })).rejects.toThrow();
    expect(sessionRepo.state.session.steps.length).toBe(0);

    // Second attempt succeeds
    const res = await h(null, { context: { turns: [] }, lastChoice: { questionId: 'q1', optionId: 'a' } });
